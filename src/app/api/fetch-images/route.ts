...
async function fetchImageAsBase64(url: string): Promise<string> {
  // Ambil gambar lewat backend proxy API kamu
  const res = await fetch(`/api/fetch-images?url=${encodeURIComponent(url)}`);
  if (!res.ok) throw new Error(`Gagal fetch image dari backend: ${res.status}`);

  const json = await res.json();
  return json.base64; // Pastikan backend return { base64: "data:image/jpeg;base64,..." }
}
      ...
      if (key === "imageUrl") {
        try {
          ...
          if (typeof value === "string" && value.startsWith("http")) {
            imageData = await fetchImageAsBase64(value); // lewat backend
          }
